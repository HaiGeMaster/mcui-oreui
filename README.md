...
把 Minecraft 基岩版界面的观感（Minecraft 像素字体、灰色立体面板、主/默认/错误三态按钮、卡片扫光、按键音效）封装为 **33 个标准 Vue 3 组件**，并附带一套 VitePress 实时文档站。
...
| 布局 | `<mc-layout>` · `<mc-header>` · `<mc-appbar>` · `<mc-appbar-button>` · `<mc-appbar-icon>` · `<mc-tabs>` · `<mc-button-tabs>` · `<mc-list>` · `<mc-list-item>` · `<mc-scroll-view>` |
